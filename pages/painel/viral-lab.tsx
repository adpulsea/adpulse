// ============================================
// AdPulse — Viral Lab (com bloqueio Pro)
// ============================================
    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative w-36 h-36">
        <svg width="144" height="144" viewBox="0 0 144 144">
          <circle cx="72" cy="72" r={raio} fill="none" stroke="var(--cor-borda)" strokeWidth="10" />
          <circle cx="72" cy="72" r={raio} fill="none" stroke={cor} strokeWidth="10"
            strokeDasharray={`${preenchido} ${circunf}`} strokeLinecap="round"
...
export default function ViralLab() {
  ...
  if (!isPro) {
    return (
      <LayoutPainel titulo="Viral Lab">
        <BloqueadoPro
          funcionalidade="Viral Lab"
          descricao="Analisa o potencial viral de qualquer hook ou ideia antes de publicares. Descobre o teu score e recebe sugestões de melhoria com IA."
          emoji="🔥"
        />
      </LayoutPainel>
    )
  ...
  const analisar = async () => {
    ...
    try {
      const r = await fetch('/api/ia/viral-lab', {
        method: 'POST',
      ...
      setProgresso(100)
      setTimeout(() => { setResultado(d); setAnimar(true) }, 300)
            ...
            <div className="flex flex-col gap-4">
              <div className="card">
                <div className="flex items-center gap-2 mb-4">
                  <TrendingUp size={18} style={{ color: 'var(--cor-marca)' }} />
                  <h3 className="font-semibold" style={{ fontFamily: 'var(--fonte-display)' }}>Analisar conteúdo</h3>
                </div>
                <div className="mb-4">
                  <label className="label-campo">Hook ou ideia de conteúdo</label>
                  <textarea value={conteudo} onChange={e => setConteudo(e.target.value)}
                    placeholder="Ex: POV: acordas às 5h da manhã por 30 dias seguidos..."
                    rows={4} className="input-campo w-full resize-none" />
                  <div className="flex flex-wrap gap-2 mt-2">
                    {CHIPS_EXEMPLO.map(c => (
                      <button key={c} onClick={() => setConteudo(c)}
                ...
                <button onClick={analisar} disabled={carregando || !conteudo.trim()}
                  ...
                  {carregando ? <><Loader size={16} className="animate-spin" /> A analisar...</> : <><TrendingUp size={16} /> Analisar viral score</>}
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {resultado ? (
                <>
                  ...
                  <div className="card">
                    <h3 className="font-semibold mb-3" style={{ fontFamily: 'var(--fonte-display)' }}>Análise por dimensão</h3>
                    <div className="flex flex-col gap-2">
                      <BarraDimensao label="🎣 Hook" valor={resultado.hook} descricao="Força do gancho inicial" />
                      <BarraDimensao label="📈 Tendência" valor={resultado.trend} descricao="Alinhamento com o viral" />
                      <BarraDimensao label="🎯 Nicho" valor={resultado.nicho} descricao="Relevância para o público" />
                      <BarraDimensao label="📐 Formato" valor={resultado.formato} descricao="Adequação à plataforma" />
                    </div>
                  </div>
                  <div className="card">
                    <h3 className="font-semibold mb-3" style={{ fontFamily: 'var(--fonte-display)' }}>Sugestões de melhoria</h3>
                    <div className="flex flex-col gap-2">
                  ...
                  <div className="card">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-semibold" style={{ fontFamily: 'var(--fonte-display)' }}>3 Hooks alternativos</h3>
                    </div>
                    <div className="flex flex-col gap-2">
                      {resultado.hooks_alternativos.map((hook, i) => (
                        <button key={i} onClick={() => setResultado(r => r ? { ...r, hookSelecionado: i } : r)}
                          className="p-3 rounded-xl text-sm text-left transition-all"
                          style={{ background: resultado.hookSelecionado === i ? 'rgba(124,123,250,0.12)' : 'var(--cor-elevado)', border: `1px solid ${resultado.hookSelecionado === i ? 'rgba(124,123,250,0.35)' : 'var(--cor-borda)'}`, color: resultado.hookSelecionado === i ? 'var(--cor-texto)' : 'var(--cor-texto-muted)' }}>
                ...
                <div className="card flex flex-col items-center justify-center text-center py-16" style={{ minHeight: 400 }}>
                  ...
                  <h3 className="font-semibold text-lg mb-2" style={{ fontFamily: 'var(--fonte-display)' }}>Descobre o teu potencial viral</h3>
                  <p className="text-sm" style={{ color: 'var(--cor-texto-muted)' }}>Analisa qualquer hook ou ideia<br />e vê o score antes de publicar.</p>
                </div>
              )}
            </div>
          </div>
