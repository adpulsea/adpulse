...
export default function GeradorImagens() {
  ...
  const gerarImagem = async (promptManual?: string) => {
    const promptFinal = (promptManual || prompt).trim()

    if (!promptFinal || gerando) return
    ...
    try {
      ...
      if (!utilizador) {
        setErro('Tens de iniciar sessão para gerar imagens.')
        setGerando(false)
        return
      }

      const limite = await verificarLimiteGeracoes(utilizador.id)

      if (!limite.podeGerar) {
        setErro(
          `Limite atingido: já usaste ${limite.geracoesHoje}/${limite.limiteMaximo} gerações hoje. Faz upgrade para continuar.`
        )
        setGerando(false)
        return
      }
      ...
      const resp = await fetch('/api/ia/gerar-imagem', {
        ...
        body: JSON.stringify({ prompt: promptFinal, tamanho, estilo, qualidade }),
      })
      ...
      await registarGeracaoAI({
        utilizadorId: utilizador.id,
        tipo: 'viral',
        promptEntrada: promptFinal,
        resultado: data.url,
        tokensUsados: 0,
      })
  ...
  return (
    <>
      <Head>
        <title>Gerador de Imagens IA — AdPulse</title>
      </Head>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                >
                  Cria imagens profissionais com DALL-E 3 — fundos, criativos,
                  fotos de produto e muito mais
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="card flex flex-col gap-3">
                ...
                <textarea
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && e.ctrlKey) gerarImagem()
                  }}
              ...
              {erro && (
                <div
                  className="px-4 py-3 rounded-xl text-sm flex flex-col gap-3"
                  style={{
              ...
              <button
                ...
                style={{
                  opacity: gerando || !prompt.trim() ? 0.6 : 1,
                  cursor: gerando || !prompt.trim() ? 'default' : 'pointer',
                }}
              >
                {gerando ? (
                  <>
            ...
            <div className="flex flex-col gap-4">
              ...
              {imagens.length === 0 && !gerando && (
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: 'var(--cor-texto-fraco)' }}
                  >
                    As tuas imagens aparecem aqui.
                    <br />
                    Escreve um prompt e clica Gerar!
                  </p>
                </div>
              )}
              ...
              {gerando && (
                <div
                  ...
                  style={{
                    border: '1px solid rgba(124,123,250,0.2)',
                    background: 'rgba(124,123,250,0.05)',
                  }}
                >
                  <div
              ...
              {imagens.map((img) => (
                <div
                  ...
                  style={{
                    border:
                      preview?.id === img.id
                        ? '1px solid rgba(124,123,250,0.4)'
                ...
                >
                  ...
                  >
                    <img
                      ...
                      style={{
                        aspectRatio:
                          img.tamanho === '1024x1792'
                            ? '9/16'
                            : img.tamanho === '1792x1024'
                              ? '16/9'
                              : '1/1',
                      }}
                    />
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      style={{
                        background: 'var(--cor-elevado)',
                        color: copiado ? '#34d399' : 'var(--cor-texto-muted)',
                        border: '1px solid var(--cor-borda)',
                        cursor: 'pointer',
                      }}
                    >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4">
                <p className="text-sm text-white opacity-70 flex-1">
                  {preview.promptRevisto || preview.prompt}
                </p>

                <div className="flex gap-2 flex-shrink-0">
                  <button
                    onClick={() => descarregar(preview)}
