// ============================================
// AdPulse — Dashboard Principal (Guia IA)
// ============================================
...
const SUGESTOES_IA: SugestaoIA[] = [
  {
    titulo: 'Tendência: "Rotina da manhã"',
    descricao: 'Alto engagement esta semana no teu nicho',
    score: 92,
    ideia: 'Rotina da manhã que ajuda criadores a ganhar consistência e foco',
  },
  {
    titulo: '"O erro que me custou 1000€"',
    descricao: 'Hooks de erro pessoal têm mais partilhas',
    score: 87,
    ideia: 'O erro que me custou 1000€ e o que aprendi com isso',
  },
  {
    titulo: 'Tutorial passo-a-passo curto',
    descricao: 'Reels de tutorial entre 30-45s estão a viralizar',
    score: 81,
    ideia: 'Tutorial rápido para criar conteúdo em menos de 10 minutos por dia',
  },
]
...
export default function Dashboard() {
        ...
        for (const post of postsHoje) {
          ...
          if (!jaNotificado) {
            await supabase.from('notificacoes').insert({
              ...
              mensagem: `"${post.titulo}" está agendado para as ${
                post.hora_publicacao || '??:??'
              } no ${post.plataforma}. Está na hora de publicar!`,
              tipo: 'aviso',
              lida: false,
  ...
  const usarSugestao = (ideia: string) => {
    localStorage.setItem('adpulse_ideia', ideia)
    window.location.href = '/painel/studio'
  }

  return (
    <>
      <Head>
        <title>Dashboard — AdPulse</title>
      </Head>
      ...
      <LayoutPainel titulo="Dashboard">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(135deg, rgba(124,123,250,0.13), rgba(192,132,252,0.08))',
              border: '1px solid rgba(124,123,250,0.24)',
            }}
          >
            <div className="flex items-start gap-3">
              ...
              >
                <Bot size={22} style={{ color: 'var(--cor-marca)' }} />
              </div>
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--cor-texto-muted)' }}>
                  Segue o fluxo recomendado pela AdPulse para criar, organizar e publicar.
                </p>
              </div>
            </div>

            <div className="grid md:grid-cols-3 gap-3">
              <Link href="/painel/studio" style={{ textDecoration: 'none' }}>
                <div
                  className="p-4 rounded-xl flex flex-col gap-3 h-full hover:scale-[1.01] transition-all"
                  style={{
                ...
                >
                  <div
                    ...
                    style={{ background: 'rgba(124,123,250,0.14)' }}
                  >
                    <Sparkles size={17} style={{ color: 'var(--cor-marca)' }} />
                  </div>

                  <p className="text-sm font-semibold">1️⃣ Gerar conteúdo</p>

                  <p className="text-xs flex-1" style={{ color: 'var(--cor-texto-muted)' }}>
                    Usa o assistente guiado para criar ideias, hooks e legendas.
                  </p>

                  <span className="text-xs font-semibold flex items-center gap-1" style={{ color: 'var(--cor-marca)' }}>
                    Criar conteúdo <ChevronRight size={12} />
                  </span>
                </div>
              </Link>

              <Link href="/painel/calendario" style={{ textDecoration: 'none' }}>
                <div
                  className="p-4 rounded-xl flex flex-col gap-3 h-full hover:scale-[1.01] transition-all"
                  style={{
                    background: 'var(--cor-elevado)',
                ...
                >
                  ...
                  >
                    <Calendar size={17} style={{ color: '#34d399' }} />
                  </div>

                  <p className="text-sm font-semibold">2️⃣ Criar plano semanal</p>

                  <p className="text-xs flex-1" style={{ color: 'var(--cor-texto-muted)' }}>
                    Organiza os conteúdos da semana e mantém consistência.
                  </p>
              ...
              <Link href="/painel/automacao" style={{ textDecoration: 'none' }}>
                <div
                  className="p-4 rounded-xl flex flex-col gap-3 h-full hover:scale-[1.01] transition-all"
                  style={{
                    background: 'var(--cor-elevado)',
                ...
                >
                  ...
                  >
                    <Rocket size={17} style={{ color: '#c084fc' }} />
                  </div>

                  <p className="text-sm font-semibold">3️⃣ Automatizar</p>

                  <p className="text-xs flex-1" style={{ color: 'var(--cor-texto-muted)' }}>
                    Ativa o funcionário IA e deixa a AdPulse trabalhar por ti.
                  </p>

                  <span className="text-xs font-semibold flex items-center gap-1" style={{ color: '#c084fc' }}>
                    Automatizar <ChevronRight size={12} />
                  </span>
                </div>
              </Link>
            </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {(metricas?.postsAgendados || 0) > 0 && (
              <div
                ...
                style={{
                  ...
                  border: '1px solid rgba(124,123,250,0.15)',
                }}
              >
                <span style={{ color: 'var(--cor-texto-muted)' }}>
                  📅 Tens {metricas?.postsAgendados}{' '}
                  {metricas?.postsAgendados === 1 ? 'post agendado' : 'posts agendados'} — não te esqueças de publicar!
                </span>
            ...
            >
              ...
              <button
                onClick={() => usarSugestao('POV dia na vida de um criador de conteúdo')}
                className="text-xs font-medium whitespace-nowrap flex items-center gap-1"
                style={{
                  color: 'var(--cor-marca)',
            ...
            {[
              ...
              return (
                <div key={m.titulo} className="card">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: 'var(--cor-texto-fraco)' }}>
                    {m.sub}
                  </p>
                </div>
              )
                ...
                {[
                  ...
                  return (
                    <Link
                      ...
                      onMouseOver={(e) => {
                        e.currentTarget.style.borderColor = a.cor + '50'
                        e.currentTarget.style.color = a.cor
                      }}
                      onMouseOut={(e) => {
                        e.currentTarget.style.borderColor = 'var(--cor-borda)'
                        e.currentTarget.style.color = 'var(--cor-texto-muted)'
                      }}
                    >
                      <div
                    ...
                    <div key={dia} className="text-center">
                      ...
                      {postsDia.length > 0 ? (
                        <div className="flex flex-col gap-1">
                          {postsDia.slice(0, 2).map((p) => (
                            <Link key={p.id} href={`/painel/publicar?id=${p.id}`}>
                              <div
                                className="aspect-square rounded-xl flex flex-col items-center justify-center p-1 gap-0.5 cursor-pointer hover:opacity-80 transition-opacity"
                                style={{
                                  background: `${COR_TIPO[p.formato] || '#7c7bfa'}20`,
                                  border: `1px solid ${COR_TIPO[p.formato] || '#7c7bfa'}40`,
                                }}
                              >
                                <span
                                  className="font-bold"
                                  style={{
                                    color: COR_TIPO[p.formato] || '#7c7bfa',
                                    fontSize: 8,
                                  }}
                                >
                                  {p.formato}
                        ...
                        <Link href="/painel/calendario">
                          <div
                            className="aspect-square rounded-xl flex items-center justify-center text-xs cursor-pointer transition-all hover:scale-105"
                            style={{
              ...
              <div className="flex flex-col gap-3">
                {SUGESTOES_IA.map((s, i) => (
                  ...
                  >
                    ...
                    <p className="text-xs mb-3" style={{ color: 'var(--cor-texto-fraco)' }}>
                      {s.descricao}
                    </p>

                    <button
                      onClick={() => usarSugestao(s.ideia)}
                      className="w-full text-xs font-semibold px-3 py-2 rounded-lg flex items-center justify-center gap-1"
                      style={{
