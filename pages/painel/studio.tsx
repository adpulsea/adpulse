...
function normalizarResultado(dados: any): ResultadoIA {
  return {
    ...
    slides: Array.isArray(dados?.slides) && dados.slides.length > 0
      ...
      : [
          {
            tipo: 'Hook',
            conteudo: typeof dados?.hook === 'string' ? dados.hook : 'Hook principal',
          },
        ],
  }
}
...
function calcularScore(hook: string, legenda: string, hashtags: string[]) {
  const hookSeguro = hook || ''
  const legendaSegura = legenda || ''
  const hashtagsSeguras = Array.isArray(hashtags) ? hashtags : []

  const scoreHook = Math.min(hookSeguro.length > 20 ? 85 + Math.random() * 15 : 40, 100)
  const scoreLegenda = Math.min(legendaSegura.length > 50 ? 75 + Math.random() * 20 : 30, 100)
  const scoreHashtags = Math.min(
    hashtagsSeguras.length >= 5 ? 80 + Math.random() * 20 : hashtagsSeguras.length * 15,
    100
  )
  const scoreCta =
    legendaSegura.toLowerCase().includes('comentar') ||
    legendaSegura.toLowerCase().includes('guardar') ||
    legendaSegura.includes('?')
      ? 85
      : 55
...
function BarraScore({ label, valor }: { label: string; valor: number }) {
  const cor =
    valor >= 80 ? 'var(--cor-sucesso)' : valor >= 60 ? 'var(--cor-aviso)' : 'var(--cor-erro)'
...
export default function StudioConteudo() {
  ...
  const score = resultado
    ? calcularScore(resultado.hook, resultado.legenda, resultado.hashtags)
    : null
  ...
  const gerarConteudo = async () => {
    ...
    try {
      ...
      if (!resposta.ok) {
        setErro(dados?.mensagem || 'Erro ao gerar conteúdo.')
        setResultado(normalizarResultado(dados))
        return
      }

      const seguro = normalizarResultado(dados)
      setResultado(seguro)
      setSlideAtual(0)
      setGeracoesHoje((prev) => prev + 1)
    } catch {
      setErro('Erro ao gerar conteúdo. Verifica a ligação.')
    } finally {
      setCarregando(false)
    }
  }
  ...
  const slidesSeguros = resultado?.slides || []
  const hashtagsSeguras = resultado?.hashtags || []
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="flex flex-col gap-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {FORMATOS.map((f) => (
                    <button
                      ...
                      style={{
                        background:
                          formato === f.id ? 'rgba(124,123,250,0.15)' : 'var(--cor-elevado)',
                        border: `1px solid ${
                          formato === f.id ? 'rgba(124,123,250,0.4)' : 'var(--cor-borda)'
                        }`,
                    ...
                    {PLATAFORMAS.map((p) => (
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${
                            plataforma === p.id ? p.cor + '40' : 'transparent'
                          }`,
                    ...
                    {TONS.map((t) => (
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${
                            tom === t ? 'rgba(124,123,250,0.3)' : 'transparent'
                          }`,
                        }}
                      >
                        {t}
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-lg">
                      {geracoesRestantes === 0 ? '🚫' : geracoesRestantes === 1 ? '⚠️' : '✨'}
                    </span>
                    ...
                    <div>
                      ...
                      >
                        {geracoesRestantes === 0
                          ? 'Limite atingido'
                          : `${geracoesRestantes} gerações restantes hoje`}
                      </p>
              ...
              {erroLimite && (
                ...
                >
                  Atingiste o limite diário do plano gratuito.{' '}
                  <a href="/precos" className="underline font-medium">
            ...
            <div className="flex flex-col gap-4">
              {resultado ? (
                <>
                  <div className="card">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setSlideAtual((s) => Math.max(0, s - 1))}
                          disabled={slideAtual === 0}
                          className="p-1.5 rounded-lg transition-colors"
                          style={{
                            color: slideAtual === 0 ? 'var(--cor-borda)' : 'var(--cor-texto-muted)',
                          }}
                        >
                          <ChevronLeft size={16} />
                        </button>
                        ...
                        <span className="text-xs" style={{ color: 'var(--cor-texto-muted)' }}>
                          {slideAtual + 1}/{slidesSeguros.length || 1}
                        </span>

                        <button
                          onClick={() =>
                            setSlideAtual((s) => Math.min((slidesSeguros.length || 1) - 1, s + 1))
                          }
                          disabled={slideAtual >= (slidesSeguros.length || 1) - 1}
                          className="p-1.5 rounded-lg transition-colors"
                          style={{
                            color:
                              slideAtual >= (slidesSeguros.length || 1) - 1
                                ? 'var(--cor-borda)'
                                : 'var(--cor-texto-muted)',
                          }}
                        >
                    ...
                    <div
                      className="rounded-xl p-5 mb-4 min-h-[120px]"
                      style={{
                        background: 'var(--cor-elevado)',
                        border: '1px solid var(--cor-borda)',
                      }}
                    >
                      ...
                      >
                        {slidesSeguros[slideAtual]?.tipo || 'Hook principal'}
                      </p>

                      <p className="text-sm leading-relaxed">
                        {slidesSeguros[slideAtual]?.conteudo || resultado.hook}
                      </p>
                    </div>

                    <div className="flex justify-center gap-1.5">
                      {(slidesSeguros.length ? slidesSeguros : [resultado]).map((_, i) => (
                        <button
                          key={i}
                          ...
                          style={{
                            ...
                            height: '6px',
                            background:
                              i === slideAtual ? 'var(--cor-marca)' : 'var(--cor-borda-clara)',
                          }}
                        />
                      ))}
                    </div>
                  ...
                  <div className="card">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={() => copiar(resultado.hook, 'hook')}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          <RefreshCw
                            size={12}
                            className={gerandoLegenda ? 'animate-spin' : ''}
                          />
                  ...
                  <div className="card">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={() => copiar(hashtagsSeguras.join(' '), 'hashtags')}
                        className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg transition-colors"
                        style={{
                          color:
                            copiado === 'hashtags'
                    ...
                    <div className="flex flex-wrap gap-2">
                      {hashtagsSeguras.length > 0 ? (
                        hashtagsSeguras.map((h, i) => (
                          <span
                            key={i}
                ...
                <div
                  className="card flex-1 flex flex-col items-center justify-center text-center py-16"
                  style={{ minHeight: 400 }}
                >
                  <div
                  ...
                  <p className="text-sm" style={{ color: 'var(--cor-texto-muted)' }}>
                    Escreve o teu tópico e clica em
                    <br />
